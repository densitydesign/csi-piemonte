...
d3.json("aziende.json", function(data) {
 ...
 var date="1982";
	...
	function switchTitle(area){
		...
		if (area=="sau"){
			...
			d3.select("chart2.testo").text("SAU");
			}
		else{
...
chart2.selectAll(".rule")
    ...
    .text(function(d){return Math.pow(d, 2) +" ha";})
    .style("fill", "gray");
...
// 	});
	...
	dDate.domain(d3.entries(data["tipo"]["1_0-0,1"]).map(function(d) { return d.key}));
...
//      .attr("y2", function(d) { return h - y2(d) ; });
   ...
   .transition()
    ...
    .tween("text", function(d) { var i = d3.interpolateRound(parseInt(this.textContent), parseInt(d)); return function(t) {this.textContent = i(t);}; });
 }
