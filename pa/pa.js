...
function scuoleSearch(elements){
		...
		var sc = d3.entries(elements.features).map(function(d){ return {label: toTitleCase(d.value.properties.DESCRIZIONE+" "+d.value.properties.DENOMINAZIONE+" ("+d.value.properties.COMUNE+")"), value:  toTitleCase(d.value.properties.DESCRIZIONE+" "+d.value.properties.DENOMINAZIONE+" ("+d.value.properties.COMUNE+")"),lat: d.value.geometry.coordinates[1],lon: d.value.geometry.coordinates[0], codice:d.value.properties["codice scuola"]};
								})
...
function init_2() {
 ...
 // Transpose the data into layers by cause.
  ...
  var rect = cause.selectAll("rect")
    ...
    .attr("opacity", 1)
      .on("mouseover", function(d, i){$("#tooltip_2 p").html(toTitleCase(d.descrizione+" - "+d.denominazione+"<br> "+toTitleCase(d.comune)+" ("+d.provincia.toUpperCase()+")")); return $("#tooltip_2").show();})
	.on("mousemove", function(){
		var w = $("#tooltip_2").width(); var left = d3.event.pageX - w/2;
       if(d3.event.pageX-w/2 < 0){left=left+Math.abs(left);}
       else if(d3.event.pageX+w/2 > $(window).width()){left=left-(w/2-($(window).width()-d3.event.pageX));}
		 var h = $("#tooltip_2").height(); return $("#tooltip_2").css({top: (d3.event.pageY - h-20) + "px", left: left + "px"});})
	.on("mouseout", function(){return $("#tooltip_2").hide();})
	.on("click", function(d){clickBar(d.data.properties["codice scuola"],d.data.geometry.coordinates[1],d.data.geometry.coordinates[0])});
...
function clickBar(codice,lat,lon){
			provincia = 0;
		comune= 0;
		$(".provincia").empty();
		$(".comune").empty();
...
function showTooltip2(data) {
 	return function(e) {
       ...
       $("#tooltip .nStudenti").text(agg_comuni.comuni[(data.properties.TOPONIMO).toUpperCase()].N_STUDENTI);
       var w = $("#tooltip").width();
       var h = $("#tooltip").height();
        var left = e.pageX - w/2;
       if(e.pageX-w/2 < 0){left=left+Math.abs(left);}
...
function showTooltip4(data) {
 	return function(e) {
       ...
       $("#tooltip h2").text(toTitleCase(data.properties.DESCRIZIONE)+" - "+toTitleCase(data.properties.DENOMINAZIONE));
